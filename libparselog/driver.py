...
class ParseDriver():
    ...
    def __init__(self, toml_file_list, import_list, preprocess_list, process_list, postprocess_list):

        # generate the conf
        self.conf = Toml(toml_file_list)

        # unload the driver entry
        driver_entries = self.conf.unload_entry("DRIVER")
    ...
    def _init_entries(self):
        for entry in self.conf:
            ...
            if self.conf[entry][_K_KEY] is None:
                self.conf[entry][_K_KEY] = False

            if self.conf[entry][_K_LIST] is None:
                self.conf[entry][_K_LIST] = False

            # if the type is a key, remove the defaults
            if self.conf[entry][_K_KEY]:
                self.conf[entry][_K_DFLT] = None
                self.conf[entry][_K_AUTO_HIDE] = False
                self.conf[entry][_K_HIDE_IF] = None

            # disable auto hide as a sane default if we have no entry
            if self.conf[entry][_K_AUTO_HIDE] is None or self.conf[entry][_K_HIDE_IF] is None:
                self.conf[entry][_K_AUTO_HIDE] = False
    ...
    def _sanitize(self):
        ...
        for entry in self:
            ...
            Toml.assertion(
                _K_REGEX in self.conf[entry],
                _K_REGEX,
                entry,
                "is required for the configuration to work",
            )
            self.Toml.assert_type(self, entry, _K_REGEX, (str, list))

            if isinstance(self.conf[entry][_K_REGEX], str):
                # regexes are always arrays, so we just fix that here
                self.conf[entry][_K_REGEX] = [self.conf[entry][_K_REGEX]]
            elif isinstance(self.conf[entry][_K_REGEX], list):
                for regexes in self.conf[entry][_K_REGEX]:
                    Toml.assertion(
                        isinstance(regexes, str),
                        _K_REGEX,
                        entry,
                        "is expected to be a list of string",
                    )

            Toml.assert_type(self, entry, _K_AUTO_HIDE, (bool))
            Toml.assert_type(self, entry, _K_LIST, (bool))
            Toml.assert_type(self, entry, _K_KEY, (bool))

            if self.conf[entry][_K_DFLT] is not None and isinstance(
                self.conf[entry][_K_DFLT], list
            ):
                self.conf[entry][_K_LIST] = True

            if self.conf[entry][_K_LIST]:
                if self.conf[entry][_K_DFLT] is None:
                    self.conf[entry][_K_DFLT] = []
                elif not isinstance(self.conf[entry][_K_DFLT], list):
                    self.conf[entry][_K_DFLT] = [self.conf[entry][_K_DFLT]]

                # make sure the hide-if is changed back too
                if self.conf[entry][_K_HIDE_IF] is not None and not isinstance(
                    self.conf[entry][_K_HIDE_IF], list
                ):
                    self.conf[entry][_K_HIDE_IF] = [self.conf[entry][_K_HIDE_IF]]

            # if we have a comparison, make sur it contains a struct we can hand off
            if self.conf[entry][_K_COMPARE] is not None:
                Toml.assert_type(self, entry, _K_COMPARE, (dict))
                Toml.assertion(
                    len(self.conf[entry][_K_COMPARE]._KEYS()) == 1,
                    entry,
                    _K_COMPARE,
                    "compare must be of format 'function: struct', with only one function",
                )
    ...
    def insert_value(self, tbl, header, value):
        if header not in tbl and self.conf[header][_K_LIST]:
            tbl[header] = []

        # append all the finds to the list
        if self.conf[header][_K_LIST]:
    ...
    def set_default(self, tbl):
        for header in self.get_header_list():
            if header not in tbl:
                tbl[header] = self.conf[header][_K_DFLT]

        return tbl

    def is_multivalued(self, header):
        return self.conf[header][_K_LIST]
    ...
    def generate_tbl(self, key: str) -> OrderedDict:
        ...
        for header in self.conf:
            if not self.conf[header][_K_KEY]:
    ...
    def generate_hidden_tbl(self) -> OrderedDict:
        ...
        return self.generate_tbl(_K_HIDE_IF)

    def regex_line(self, header, line):
        # compile the regex entries
        entry_list = []

        for regexes in self.conf[header][_K_REGEX]:
    ...
    def auto_hide_values(self, dataset):
        for header in self.conf:
            if self.conf[header][_K_AUTO_HIDE] is True and header in dataset:
                if (
                    dataset[header] is self.conf[header][_K_HIDE_IF]
                    or dataset[header] == self.conf[header][_K_HIDE_IF]
                ):
                    del dataset[header]
    ...
    def compare(self, header, expected, got):
        if self.conf[header][_K_COMPARE] is None:
            return True

        return self.comparator.compare(
            list(self.conf[header][_K_COMPARE].keys())[0],
            list(self.conf[header][_K_COMPARE].values())[0],
            expected,
            got,
        )
